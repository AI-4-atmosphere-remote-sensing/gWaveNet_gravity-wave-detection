...
model.add(layers.Conv2D(1, (5, 5), activation='relu', kernel_initializer=custom_layer, trainable=False, padding='same', input_shape=(200, 200, 1)))
model.add(layers.MaxPool2D(pool_size=(2,2), padding='same', strides=(2,2)))

model.add(layers.Conv2D(512, (5, 5), activation='relu', padding='same', kernel_regularizer = regularizers.l2(1e-4)))
...
with open('./trainHistory', 'wb') as file:
    pickle.dump(h.history, file)

# with open('./trainHistory', "rb") as file:
...
plt.show()
plt.draw()
# fig1.savefig('cm_test.pdf', dpi=300)
# fig1.savefig('cm_test', dpi=300)
