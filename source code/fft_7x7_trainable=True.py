...
model.add(layers.Conv2D(1, (7, 7), activation='relu', kernel_initializer=custom_layer, trainable=True, padding='same', input_shape=(200, 200, 1))) 
...
# fit model
checkpoint = keras.callbacks.ModelCheckpoint('saved_mdoel{epoch:04d}.h5', period=500)
h = model.fit(train_generator, epochs = 2000, validation_data=validation_generator, callbacks=[checkpoint])
model.save_weights('l1-04-apr-2.h5')
